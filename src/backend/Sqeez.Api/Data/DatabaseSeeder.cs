...
namespace Sqeez.Api.Data
{
    public static class DatabaseSeeder
    {
        public static async Task SeedAsync(SqeezDbContext context, IConfiguration config)
        {
            ...
            var uploadsFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "uploads", "media");
            if (!Directory.Exists(uploadsFolder))
            {
                Directory.CreateDirectory(uploadsFolder);
            }

            var sampleFileName = "seed-sample-image.txt";
            var physicalPath = Path.Combine(uploadsFolder, sampleFileName);

            if (!File.Exists(physicalPath))
            {
                await File.WriteAllTextAsync(physicalPath, "This is a sample media file generated by the Database Seeder.");
            }
